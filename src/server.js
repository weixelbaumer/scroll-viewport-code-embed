...
// Render GitHub macro endpoint - Serves the static HTML for the iframe
app.get('/render-github-macro', (req, res) => {
    logDebug(`Serving static file for /render-github-macro request: ${JSON.stringify(req.query)}`);
    // This endpoint simply serves the HTML file.
    // The client-side JS within this HTML file will use AP.confluence.getMacroData
    // to get parameters and then call the /html endpoint to fetch the actual code.
    res.sendFile(path.join(__dirname, '../atlassian-connect/macro-render.html'));
});
